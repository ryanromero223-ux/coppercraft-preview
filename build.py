...
/* award laurels — strip above the press bar */
.laurels{display:flex;justify-content:center;align-items:center;gap:48px;flex-wrap:wrap;padding:10px 20px 34px}
.laurels img{height:76px;width:auto}
@media(max-width:520px){.laurels{gap:26px;padding-bottom:28px}.laurels img{height:60px}}
...
(function(){
  ...
  var cart=loadCart();
  /* carry ad UTMs (and fbclid) through to the Shopify checkout so orders
     are attributable in the Flaviar/BarCart dashboard */
  var UTM='';
  try{
    if(location.search.length>1) sessionStorage.setItem('ccUTM',location.search.slice(1));
    UTM=sessionStorage.getItem('ccUTM')||'';
  }catch(e){}
  function withUTM(url){ return UTM ? url+(url.indexOf('?')>-1?'&':'?')+UTM : url; }
  ...
  function checkoutCart(){
    ...
    fetch('https://__STORE__/api/2024-01/graphql.json',{
      ...
      if(url){ window.location.href=withUTM(url); } else { fallback(keys); }
  ...
  function fallback(keys){
    if(keys.length===1 && keys[0]!=='rye'){
      window.location.href=withUTM('https://__STORE__/cart/'+VARIANTS[keys[0]]+':'+cart.items[keys[0]]);
    }else{
